...
w_stat <- adsl_eff %>%
  group_by(TRT01AN,TRT01A,SEX) %>%
  summarize(mean = (mean(WEIGHTBL) %>% round(digits = 1) %>% format(nsmall=1)),
            sd = (sd(WEIGHTBL) %>% round(digits = 1) %>% format(nsmall = 1)), 
            med = (median(WEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)),              
            min = (min(WEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)), 
            max = (max(WEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)),
            n = n()) %>% 
  mutate(range_minmax = paste0("(",min, ",", max,")"))
# w_stat

w_desc_stat_long <- w_stat %>%
  ungroup() %>%
  select("TRT01A","SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","SEX"), names_to ="category", values_to = "values" )
...
h_stat <- adsl_eff %>%
  ...
  summarize(mean = (mean(HEIGHTBL) %>% round(digits = 1) %>% format(nsmall=1)),
            ...
            med = (median(HEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)),              
            min = (min(HEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)), 
            max = (max(HEIGHTBL) %>% round(digits = 0) %>% format(nsmall=0)),
            n = n()) %>% 
  mutate(range_minmax = paste0("(",min, ",", max,")"))
# h_stat


h_desc_stat_long <- h_stat %>%
  ungroup() %>%
  select("TRT01A","SEX", "n", "mean", "med", "sd", "range_minmax") %>% 
  mutate(across(where(is.numeric), .fns = as.character)) %>%
  pivot_longer(-c("TRT01A","SEX"), names_to ="category", values_to = "values" )
