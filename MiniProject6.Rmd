...
plot2b <- ALT2 %>%
  filter(WEEK %in% c(0, 5, 10, 15, 20, 25, 30)) %>%
  ggplot(mapping = aes(x = as.factor(WEEK) , y = LBSTRESN, fill = as.factor(WEEK))) +
...
p2b <- p2 + plot_annotation(
  title = unique(ALT$STUDYID) ,
  subtitle =  "Plot of ALT Measurements during Active Study Period",
...
plot_challenge <- ALT3 %>%
  ...
  geom_point(data = ALT3_outrange, mapping = aes(x = LBSTRESN, color = "red")) +
