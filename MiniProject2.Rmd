...
Notice in this output that the column `perc` is of type `<dbl>` while `perc_char` is of type `<char>`. Anything that is character type in R will not be further formatted and will display "as is" in a table or listing.
...
Big_N_cnt_saf <- adsl_saf %>%
  		group_by(TRT01AN, TRT01A, AGEGR1N, AGEGR1) %>%
  		count(name = "N")
Big_N_cnt_saf

small_n_cnt_saf <- adsl_saf %>%
  		group_by( TRT01AN, TRT01A, AGEGR1N, AGEGR1, SEX) %>%
  		count(name = "n")
small_n_cnt_saf

adsl_mrg_cnt_saf <- small_n_cnt_saf %>% 
                    left_join(Big_N_cnt_saf, by = c("TRT01A", "TRT01AN", "AGEGR1", "AGEGR1N")) %>%
                    mutate(perc = round(n/N*100, digits=1)) %>%
                    mutate(perc_char = format(perc, nsmall=1)) %>%
                    mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
                    mutate(SEX = recode(SEX, "M" = "Male", "F" = "Female")) %>%
...
Generate Counts and Percentages of Body Text Term (AEBODSYS) for Safety Population (SAFFL) based on this CDISC dataset: `adae.xpt`
```{r}
adae <- import( file = "https://github.com/phuse-org/phuse-scripts/raw/master/data/adam/cdisc/adae.xpt"  )

adae_saf <- filter(.data = adae, SAFFL == "Y")

Big_N_cnt_saf <- adae_saf %>%
  		group_by(TRTAN, TRTA) %>%
  		count(name = "N")
Big_N_cnt_saf

small_n_cnt_saf <- adae_saf %>%
  		group_by( TRTAN, TRTA, AEBODSYS) %>%
  		count(name = "n")
small_n_cnt_saf

adae_mrg_cnt_saf <- small_n_cnt_saf %>% 
                    left_join(Big_N_cnt_saf, by = c("TRTA", "TRTAN")) %>%
                    mutate(perc = round(n/N*100, digits=1)) %>%
                    mutate(perc_char = format(perc, nsmall=1)) %>%
                    mutate(npct = paste(n, "(", perc_char, ")" ) ) %>%
                    ungroup() %>%   
