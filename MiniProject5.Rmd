...
```{r}
plot2 <- plot1 + 
  labs(title = "Pulse Rates during Active Treatment Period",
...
plot4.2 <- plot3 +
  facet_grid(rows = vars(TRTA))
plot4.2

plot4.3 <- plot3 +
  facet_grid(cols = vars(TRTA))
plot4.3

plot4.4 <- plot3 +
  facet_grid(cols = vars(TRTA), rows = vars(SEX))
plot4.4
```
...
```{r}
plot3 + 
  stat_summary(geom = "point", fun = median, color = "lightgreen")
```
...
```{r}
plot3 +
  stat_summary(mapping = aes(group = NULL), fn = median, colour = "lightgreen")
```
...
```{r}
create_plot <- function(plot_template, paramn_val){
  param_data <- ADVS  %>%
    filter(PARAMN == {{paramn_val}}) %>%
    filter(VISITNUM > 3) 
...
DiasBP_plot <- create_plot(plot4, 2)
Temp_plot <- create_plot(plot4, 6)
...
```{r}
create_plot <- function(data, plot_template, param, active_period){
  param_data <- {{data}}  %>%
    filter(PARAM == {{param}}) %>%
    filter(VISITNUM >= {{active_period}}) 
...
ADVS_2 <- import("adam_advs_data_neurology.xpt")
PulseRate_plot <- create_plot(ADVS_2, plot4, "Heart Rate", 4)
PulseRate_plot

#Add as function inputs: dataset, start of active treatment period
#Filter PARAM instead of PARAMN since latter is not in the sample dataset
