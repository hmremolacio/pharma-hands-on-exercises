...
```{r dm age cat ordered}
dm_allcomb_reord <- bind_rows(agestat_cat[1,], age_cat_ordered, agestat_cat[c(2:nrow(agestat_cat)),]) %>% 
  add_row(category="AGE", .before=2)
dm_allcomb_reord
```
...
ethnic_pct <- ethnic_data_new %>%
  mutate(perc_tchar = format(perc_tot, nsmall = 1)) %>%
  mutate(perc_echar = format(perc_ethnic, nsmall = 1))

ethnic_n_pct <- ethnic_pct %>%
  mutate(npct = paste(ethnic_n, paste0("(", perc_echar, ")"))) %>% 
...
ethnic_cat <- rename(Ethnic_trans, category=ETHNIC) %>%
  arrange(category) %>% 
  add_row(category="ETHNICITY", .before=1)
...
#Calculate counts for groups
race_N_cnt <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, RACEN, RACE) %>%
  count(name = "race_total")

race_n_cnt <- adsl_eff %>%
  group_by(TRT01AN, TRT01A, SEX, RACEN, RACE) %>%
  count(name = "race_n")

race_mrg_cnt <- race_n_cnt %>% 
  left_join(race_N_cnt, 
            by = c("TRT01AN", "TRT01A", "RACEN", "RACE"))
...
race_cat <- rename(Race_trans, category=RACE) %>% 
    add_row(category="RACE", .before=1)
...
dm_final <- bind_rows(dm_allcomb_reord, ethnic_cat, race_cat)
dm_final
