...
First, let's write some tests for our `wrangle_data()` function. For our test data we are going to use the CDISC ADaM data published by PHUSE organization (`adsl_data` is already loaded into our environment). The rule of thumb is to use different data for testing than the data you are using to run the production code. The data should also be well controlled. In this case, the ADSL data format is standardized coming from a reliable source. The `wrangle_data()` function is doing a couple of transformations (it filters and subsets the data) and we want to test for unexpected **output**.
...
test_that("calculate_small_n_cnt() output has expected dim, columns, and types", {
  ...
  expect_equal(dim(test_small_n_cnt), c(length(unique(test_small_n_cnt$TRT01A))*length(unique(test_small_n_cnt$SEX)), 4))
